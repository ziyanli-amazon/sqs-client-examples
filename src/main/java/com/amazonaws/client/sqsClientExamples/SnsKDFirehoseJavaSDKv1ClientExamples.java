...
public class SnsKDFirehoseJavaSDKv1ClientExamples {
    ...
    static String deliveryStreamName = "sns-firehose-delivery-stream";
    static String bucketName = "sns-firehose-delivery-stream-bucket";
    static String SNSFirehoseRoleName = "SNS-Firehose-Role";
    static String firehoseS3RoleName = "Firehose-S3-Role";
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        AmazonIdentityManagement iamClient = AmazonIdentityManagementClientBuilder.standard().withRegion(REGION).build();
        ...
        /**
         * Step 1:
         * a. Create S3 bucket as the destination for the Kinesis Firehose delivery stream;
         * b. Create an IAM role with firehose granting permission to s3,
         * so firehose data stream can be written into an s3 object;
         * c. Configure s3 buffer conditions;
         * d. Configure ExtendedS3DestinationConfiguration for firehose data stream.
         * */
        s3Client.createBucket(bucketName);
        bucketArn = "arn:aws:s3:::" + bucketName;
        System.out.println("bucket Arn: " + bucketArn);

        // Create IAM role with firehose granting permission to s3 so firehose data stream can be written into an s3 object.
        String firehoseS3RoleArn = createIamRole(iamClient, firehoseS3RoleName, getFirehoseAssumedRoleDoc(), getFirehoseS3Policy());
        ...
        /**
         * Step2:
         * Create firehose data stream with the ExtendedS3DestinationConfiguration defined previously.
         */
        ...
        try {
            ...
            System.out.println(e.getErrorMessage() + " \n Skipping creating firehose dalivery data stream...");
        }

        // Describe the delivery stream to fetch the streamArn
        DescribeDeliveryStreamRequest describeDeliveryStreamRequest = new DescribeDeliveryStreamRequest()
                .withDeliveryStreamName(deliveryStreamName);
        DescribeDeliveryStreamResult describeDeliveryStreamResult = firehoseClient.describeDeliveryStream(describeDeliveryStreamRequest);
        deliveryStreamArn = describeDeliveryStreamResult.getDeliveryStreamDescription().getDeliveryStreamARN();
        System.out.println("Delivery Stream arn: " + deliveryStreamArn);

        String deliveryStreamStatus = describeDeliveryStreamResult.getDeliveryStreamDescription().getDeliveryStreamStatus();
        System.out.println("The firehose data stream is in status: " + deliveryStreamStatus);

        /**
         * Please allow 5-10 min for the firehose delivery data stream to be created.
         * Only perform the follwoing actions when the delivery data stream's status is -Active-.
         */
        ...
        /**
         * Step 3:
         * a. Create another IAM role, which grants access for SNS to call respective firehose APIs like PutRecord, etc.
         * b. Subscribe firehose data stream created previously to the SNS topic
         */

        // Create an IAM role to grant SNS access to firehose resources.
        String SNSFirehoseRoleArn = createIamRole(iamClient, SNSFirehoseRoleName, getSnsAssumedRoleDoc(), getSnsFirehosePolicy(deliveryStreamArn));
        ...
        /**
         * Step 4:
         * a. Publish a message to SNS topic
         * Supposedly, this message will be transferred via firehose data stream to S3 bucket created previously.
         * b. Verify that we got the message in S3 bucket.
         */
        ...
        assert(keys.size() == 1); // there should be only one object in the bucket (the message in S3 bucket)

        // Verify that we got the message in S3 bucket.
        GetObjectRequest getObjectRequest = new GetObjectRequest(bucketName, keys.get(0));
        try {
            ...
            // Verify the content
            System.out.println("Content of the object:");
    ...
    private static String createIamRole(AmazonIdentityManagement iamClient, String iamRoleName, String assumedRolePolicyDoc, String rolePolicyDoc) {
        ...
        try {
            ...
            System.out.println("IAM role '" + iamRoleName + "' does not exist. Continue to create one for it...");
        }
    ...
    private static void deleteResources(AmazonSNS snsClient, AmazonIdentityManagement iamClient, AmazonKinesisFirehose firehoseClient) {
    ...
    // Delete IAM roles
    DeleteRoleRequest deleteRolePolicyRequest = new DeleteRoleRequest().withRoleName(SNSFirehoseRoleName);
    ...
    DeleteDeliveryStreamRequest deleteDeliveryStreamRequest = new DeleteDeliveryStreamRequest().withDeliveryStreamName(deliveryStreamName);
